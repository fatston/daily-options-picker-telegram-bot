...
const { extractPicksFromBrief, parsePickCsv, readPickCsv } = require("../src/picks");
...
test("config reads publish settings", () => {
  ...
  assert.strictEqual(config.picksFile, path.join(dir, "data", "picks.csv"));
...
test("publishBrief sends to subscribers and skips duplicate ids", async () => {
  ...
  assert.deepStrictEqual(first, { sent: 2, skipped: false, picks: 0, csv: { added: 0, total: 0 } });
  assert.deepStrictEqual(second, { sent: 0, skipped: true });
  assert.strictEqual(sent.filter((message) => message.text === "brief").length, 2);
});

test("publishBrief records structured picks in local CSV", async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dop-"));
  const { bot } = createBot({
    config: {
      adminChatId: "42",
      picksFile: path.join(dir, "picks.csv"),
      reportReadyTime: "08:35",
      reportTimezone: "America/New_York",
      reportWeekdays: [1, 2, 3, 4, 5]
    }
  ...
  const result = await bot.publishBrief({
    message: "brief",
    publishId: "2026-05-28",
    picks: [{ ticker: "SNOW", size: "large", direction: "call" }]
  });

  assert.deepStrictEqual(result.csv, { added: 1, total: 1 });
  assert.deepStrictEqual(parsePickCsv(readPickCsv(bot.config.picksFile)), [
    { date: "2026-05-28", ticker: "SNOW", size: "large", direction: "call" }
  ]);
});

test("brief text can be parsed into pick rows", () => {
  const brief = [
    "Small cap ticker - ONDS",
    "Bull - CALL",
    "Mid cap ticker - PLAB",
    "Bear - PUT",
    "Large cap ticker - SNOW",
    "Bull - CALL"
  ].join("\n");

  assert.deepStrictEqual(extractPicksFromBrief("2026-05-28", brief), [
    { date: "2026-05-28", ticker: "ONDS", size: "small", direction: "call" },
    { date: "2026-05-28", ticker: "PLAB", size: "med", direction: "put" },
    { date: "2026-05-28", ticker: "SNOW", size: "large", direction: "call" }
  ]);
});
